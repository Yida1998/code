"""
Author: yida
Time is: 2022/4/21 11:21 
this Code: 批量使用KFold, 看准这个输入是什么, 正常的操作是KFold, 谁叫你代码备注不显著 沃日 应该是一个文件夹下/多个文件夹/类别/数目
"""
import os
import shutil
import time
...
if __name__ == '__main__':
    # ==================设置超参数===================== #
    # 有很多数据集的一个文件夹
    root = "/Users/yida/Desktop/high/0/val/暗紫泥大泥土"
    #  save_path:存放数据路径
    save_path_ = "/Users/yida/Desktop"
    #  k_num : 设置交叉验证数
    k_num = 4
    root_file = os.listdir(root)
    for root_file_item in root_file:
        #  root_data : 数据路径    格式:文件名->n个类别->m张图像(需符合pytorch读取训练数据集要求)
        root_data = os.path.join(root, root_file_item)  # 遍历每一个文件夹
        if os.path.isdir(root_data):
            ...
            save_path_temp = save_path_     # 初始化文件夹, 不然一直叠加目录了
            save_path = os.path.join(save_path_temp, root_data.split('/')[-1])
