...
this Code: 重新开始整理trainBatch, 基础代码是选择的trainBatch为架构, 然后根据trainBatch进行修改

2022年04月09日23:59:15
--------------------------最终说明---------------------------
注意: -> 批量训练交叉验证集 ; train.py应与该文件保持, 新增功能时应该回到train.py增加相同功能
root_path: 交叉验证训练集路径, data/k折[0,k-1]/trian-val
test_path: 测试集路径, 最终保存模型的测试路径
-----------------------------------------------------------

2022年03月20日21:13:28
从头到尾改了, 以trian为基础对着原始trainBatch修改的, 感觉没有修改很多东西, 幸好原来改了不少
明天把测试修改完, 就整理数据

2022年03月29日16:35:06
1.已经修改完成最终的测试代码
问题说明:
第一条:with open 为什么可以不用缩进 -> 必须在with里面, 不用的话会报错的
第二条:把需要手动设置的类标签, 通过dataloader设置成自定义的 -> 已经做了

整体修改一下代码, 看看有无漏洞
2022年03月29日21:10:51 -> 代码初步已全部完成, 接下来的时间看一下有无错误, 调试一下代码, 进入最终的训练阶段.

自己感觉有问题, 但是原来的自己想得真的很周到, 我还以为自己出错了, 结果没有错. 很棒 益达 一鼓作气 冲.

2022年03月30日19:29:28
整体来说没什么问题, 运行也是, 看一下论文, 把所需要的模型弄好就OK, 慢慢进行下一步
...
from Model.efficientnet import efficientnet_b0  # efficientnet_b0
...
def initialization():
    ...
    # 初始化文件夹
    UID = time.strftime('/%m%d/' + opt.root_path.split('/')[-1] + '/%H%M')  # 唯一标识符
...
def choseModel(model_name):
    ...
    if model_name == 'resnet':
        ...
        return efficientnet_b0(num_classes=opt.n_class)
    else:
...
def Summarize():
    ...
    return np.mean(acc_summarize)  # 返回times次训练结果的均值
...
if __name__ == '__main__':
    ...
    parser.add_argument('--KFold', type=int, default=5, help='K折交叉验证划分数据集数')
    parser.add_argument('--root_path', type=str, default='/Users/yida/Desktop/未命名文件夹/raw',
                        help='对该数据集进行K折交叉验证, 对每个验证集进行验证')
    parser.add_argument('--test_path', type=str, default='/Users/yida/Desktop/未命名文件夹/raw', help='模型最终测试的测试集路径')
    ...
    parser.add_argument('--batchSize', type=int, default=128, help='模型训练的Batch_size')
    ...
    # 开始k折交叉训练
    for kVal in range(opt.KFold):
        # 定义全局变量
        acc_global = []  # 统计训练n次的平均准确率, 这个不能放在全局, 不然统计的是很多次的, 有问题
        train_path = os.path.join(opt.root_path, str(kVal), 'train')  # 训练集路径
        val_path = os.path.join(opt.root_path, str(kVal), 'val')  # 验证集路径
        ...
        with open(varSave + '/result.txt', 'w') as f:
            # 初始化
            start = time.time()
            print("-------------------------trainBatch数据集:{}  KFold={}-------------------------".format(
                opt.root_path.split('/')[-1], opt.KFold))
            print("")
            f.write("-------------------------trainBatch数据集:{}  KFold={}-------------------------\n".format(
                opt.root_path.split('/')[-1], opt.KFold))
            f.write("\n")
            # 获取数据集和验证集路径
            print("------------------------------KFold={}------------------------------".format(kVal + 1))
            f.write("------------------------------KFold={}------------------------------\n".format(kVal + 1))
            # 基本信息写入txt
            temp_write()
            for k in range(opt.train_times):
                ...
                main()
            sMean = Summarize()  # 输出一个数据集多次实验的均值
    ...
    np.savetxt(varSave + '/KFoldSummary.txt', Summary, fmt='%.4f')  # 保存结果到txt
    with open(varSave + '/KFoldSummary.txt', 'a') as f2, open(varSave + '/result.txt',
                                                              'a') as f:  # 追加最终结果到txt以及KFoldSummary
        f2.write("{:.4f}±{:.4f}".format(np.mean(Summary), np.std(Summary)))
        ...
        print("k={}时在验证集上有最好准确率为:{} 其最佳的模型编号为:{} 最佳准确率:{}".format(best_k, Summary[best_k], best_model,
                                                                  ModelNum_list[best_k][best_model]))
        f.write("k={}时在验证集上有最好准确率为:{} 其最佳的模型编号为:{} 最佳准确率:{}\n".format(best_k, Summary[best_k], best_model,
                                                                      ModelNum_list[best_k][best_model]))
